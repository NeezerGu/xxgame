...
## M5 阶位线/阶段解锁（主线）
- 验收标准：
  - 新增阶位主线配置表，至少包含 5 个阶段节点，每个节点有前置与奖励（产出上限、委托类型或槽位）。
  - UI 可查看当前阶位、下一阶段要求与奖励，进度可验证（完成条件后可解锁）。
  - 阶位与升华协同：升华重置至初始阶位，但保留研究与洞察，UI 提供预览说明。
  - 存档包含阶位进度并通过迁移工具验证兼容。
  - 当前实现：提供 6 个原创阶位，晋升条件覆盖精华累积、委托完成与声望，UI 支持进度显示与禁用原因，升华后回到初始阶位且迁移向后兼容。
...
- 验收标准：
  ...
  - 当前实现：资源改为可扩展 Record，新增 herb/ore 材料并可通过委托获取，UI 与导出/基线同步展示。

## M7 炼金/锻造/装备（核心系统堆叠）
- 验收标准：
  - 炼金：至少 3 个丹方，具备成本、时长、产出与品质权重；支持队列化生产与离线完成，产物存入库存。
  - 锻造：定义器胚/部位/词条表，锻造流程可复用队列，输出装备并写入存档。
  - 装备：角色/符匣界面展示装备位，支持穿戴/卸下/强化，强化公式数据驱动且确定性。
  - 研究/阶位与上述系统产生可见联动（如加速、成功率、额外词条），相关数据表与 UI 文案同步。
  - 当前实现：装备底座与锻造队列已上线，包含四个槽位的原创蓝图与词缀定义，穿戴/卸下可影响产出乘区、委托速度与离线上限；锻造基于 seed 可复现的稀有度/词缀，并可分解返还材料，UI 展示加成与进度。

## M8 学徒/外勤探索（自动化与探索）
- 验收标准：
  - 学徒实体上线：生成规则确定性（seed），包含基础属性与专精，存档可迁移。
  - 派遣系统：可将学徒派往炼金/锻造/外勤探索，派遣过程对产出或成功率有可量化影响。
  - 外勤探索：以节点图或任务列表呈现，含时长、风险、奖励表，支持离线推进与结算。
  - 自动化：通过研究或阶位解锁自动排产/派遣选项，并可在 UI 切换，具备最小防呆提示。
