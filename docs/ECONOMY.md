...
- 岗位效果：
  - 契约书记：自动领取完成的契约并按评分自动接单。
  - 炼器匠人：锻造速度乘区 `1 + 0.15 × 资质`。
  - 炼丹学徒：预留相同 0.15 × 资质 的炼丹加速乘区（待配方上线）。
  - 采集员：被动产出 `herb/s = 0.05 × 资质`，`ore/s = 0.04 × 资质`，可叠加。

## 历练/秘境奖励
- 每个历练定义时长与掉落表（资源/配方/器胚线索），tick 推进时按事件间隔触发风险或奖励事件。
- 掉落按权重表抽取 `rewardRolls` 次，结果立即入库；配方会加入 `realm.unlockedRecipeIds`，器胚线索暂折算为等额灵矿。
- 随行弟子：契约书记/采集员可减轻负面事件，炼器/炼丹岗位可放大额外掉落次数。
