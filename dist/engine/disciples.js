...
export function recruitDisciple(state) {
...
export function assignDiscipleRole(state, discipleId, role) {
...
export function getDiscipleModifiers(state) {
    return state.disciples.roster.reduce((acc, disciple) => {
        if (!disciple.role)
            return acc;
        ...
        if (!effect)
            return acc;
        if (effect.autoClaim)
            acc.autoClaimContracts = true;
        if (effect.autoAccept)
            acc.autoAcceptContracts = true;
...
export function applyDiscipleGathering(state, dtMs, modifiers) {
...
function claimCompletedContracts(state) {
    let next = state;
    ...
    while (true) {
        const candidates = CONTRACT_DEFINITIONS.flatMap((def) => {
            ...
            if (!slot || slot.status !== "idle")
                return [];
            if (!canAcceptContract(next, def.id))
                return [];
            return [
                {
                    def,
